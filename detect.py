...
import pyrealsense2 as rs  # https://github.com/IntelRealSense/librealsense/issues/5275

# https://github.com/35selim/RealSense-Jetson/blob/main/initialize_Jetson.sh
...
def run(
    ...
    model = Detections(
        weights=os.path.join(os.getcwd(), "bestcott.pt"),
        device=device,
        dnn=dnn,
        data=data,
        fp16=half,
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        zDepth = depth.get_distance(
                            int(d1), int(d2)
                        )  # by default realsense returns distance in meters
                        tl = 3
                        tf = max(tl - 1, 1)  # font thickness
                        cv2.putText(
                            im0,
                            str(round((zDepth * 39.3701), 2))
                            + "in "
                            + str(round((zDepth * 100), 2))
                            + " cm",
                            (d1, d2),
                            0,
                            tl / 3,
